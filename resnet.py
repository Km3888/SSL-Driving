...
class ResNetEncoderFeatures(nn.Module):

    def __init__(self, block, layers,output_channels, num_classes=1000, zero_init_residual=False):
        super(ResNetEncoderFeatures, self).__init__()
        self.inplanes = 64
        ...
        self.layer7 = self._make_layer(block, output_channels, layers[6], stride=2)
    ...
    def forward(self, x):
        ...
        feats1 = self.layer1(conv1)
        feats2 = self.layer2(feats1)
        feats3 = self.layer3(feats2)
        feats4 = self.layer4(feats3)
        feats5 = self.layer5(feats4)
        feats6 = self.layer6(feats5)
        feats7 = self.layer7(feats6)


        return feats1, feats2, feats3,feats4,feats5,feats6,feats7
...
def resnet18_encoder(output_channels,pretrained=False,**kwargs):
    model = ResNetEncoderFeatures(BasicBlock, [2, 2, 1, 1, 1, 1, 1],output_channels, **kwargs)
    if pretrained:
